...
async def consultar_ley(
    ...
    if articulo:
        ...
        for art_obj in articulos_data_original: 
            if art_obj.articulo_id_interno == articulo_buscado_norm: articulo_encontrado_obj = art_obj; break
        if not articulo_encontrado_obj: 
            ...
            for art_obj in articulos_data_original:
                if patron_texto.search(art_obj.texto): art_obj.nota_busqueda = f"Encontrado por mención de '{articulo}' (normalizado a '{articulo_buscado_norm}') en texto."; articulo_encontrado_obj = art_obj; break 
        if articulo_encontrado_obj: articulos_a_devolver = [articulo_encontrado_obj]
        else: 
        ...
        else: articulos_a_devolver = articulos_data_original
    return LeyDetalle(
        ley=numero_ley, id_norma=id_norma, articulos_totales_en_respuesta=len(articulos_a_devolver), 
        articulos=articulos_a_devolver, total_articulos_originales_en_ley=total_articulos_originales_resp if nota_truncamiento_lista_resp else len(articulos_a_devolver), 
