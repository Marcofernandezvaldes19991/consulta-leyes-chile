...
app = FastAPI(
    ...
    version="2.1.0", # Versión actualizada tras simplificación
    servers=[
        {
...
# Ya no se necesita ArticuloHTML
...
#  y extraer_articulos se mantienen igual que en la versión v20. 
#  Se omiten aquí por brevedad pero deben estar presentes en el archivo .py final)
#  Asegúrate de copiar las versiones completas de estas funciones desde el artefacto v20.
...
@app.get(
    "/ley", 
    response_model=LeyDetalle, # El endpoint siempre devolverá LeyDetalle
    summary="Consultar Ley por Número y Artículo (Opcional)",
    description="Obtiene los artículos de una ley chilena. Si se especifica un artículo, la lista de artículos en la respuesta contendrá solo ese artículo (si se encuentra)."
)
async def consultar_ley(
    ...
    if articulo:
        ...
        if not articulo_encontrado_obj: # Si no se encontró por ID, intentar búsqueda textual
            ...
            try:
                ...
                logger.exception("Error compilando regex para búsqueda textual.")
        ...
        if articulo_encontrado_obj:
            ...
            logger.warning(f"Artículo '{articulo}' (buscado como '{articulo_buscado_norm}') no encontrado en ley {numero_ley}. IDs disponibles: {ids_internos[:10]}")
    ...
    else: # Si no se especificó un artículo, devolver la lista (posiblemente truncada)
...
# El endpoint /ley_html se ha eliminado.
