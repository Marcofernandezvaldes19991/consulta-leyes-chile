...
from cachetools import TTLCache # Para el caché simple con TTL (Time To Live)
...
app = FastAPI(
    ...
    version="1.9.0", # Versión actual del código
    servers=[
        {
...
MAX_TEXT_LENGTH = 10000  # Límite de caracteres para el texto de un artículo
...
def normalizar_numero_articulo_para_comparacion(num_str: Optional[str]) -> str:
    ...
    s_antes_palabras = s
    for palabra, digito in WORDS_TO_INT.items(): # Para palabras numéricas dentro de frases más largas
    ...
    s = s.strip().rstrip('-').strip() # Nueva limpieza de guiones
    ...
    for num_part, letra_part in partes_numericas:
        ...
        # Esto es una simplificación; un parser más avanzado usaría índices de las coincidencias.
...
def extraer_referencias_legales_mejorado(texto_articulo: str) -> List[str]:
    ...
    patrones = [
        ...
        r"decreto\s+con\s+fuerza\s+de\s+ley\s+(?:N(?:[°ºªo]|\.?)?|número)?\s*[\d\.]+(?:,\s*de\s+\d{4}(?:,\s*d[e|el]\s*Ministerio\s+d[e|el][\s\w]+(?:\s+y\s+[\s\w]+)*)?)?",
        ...
        r"inciso\s+\w+\s+del\s+art[íi]culo\s+[\w\d]+(?:\s*bis)?", # Referencia a inciso de un artículo (podría ser de la misma ley)
    ...
    for patron in patrones:
        try:
            ...
            for match_obj in re.finditer(patron, texto_norm_espacios, re.IGNORECASE):
                ...
                # Limpiar un poco la referencia encontrada
...
async def obtener_id_norma_async(numero_ley: str, client: httpx.AsyncClient) -> Optional[str]:
    ...
    if not norm_numero_ley_buscado.isdigit(): 
        logger.warning(f"El número de ley '{numero_ley}' (normalizado a '{norm_numero_ley_buscado}') no es puramente numérico. Verificando fallbacks.")
    ...
    for attempt in range(max_retries):
                ...
                if numero_norma_tag and numero_norma_tag.text:
                    ...
                    if numero_norma_en_xml == norm_numero_ley_buscado:
                        ...
                        if id_norma_tag and id_norma_tag.text:
                            ...
                            logger.info(f"ID encontrado para ley '{norm_numero_ley_buscado}': {id_encontrado}.")
                            cache_id_norma[norm_numero_ley_buscado] = id_encontrado 
                            return id_encontrado
                        else:
                            logger.warning(f"Coincidencia de número de ley '{numero_norma_en_xml}', pero no se encontró IdNorma.")
            logger.warning(f"No se encontró un ID de norma coincidente para la ley '{norm_numero_ley_buscado}'.")
            return None 
        ...
        if attempt < max_retries - 1: 
            ...
            return None
    return None # Debería ser inalcanzable si el bucle termina
...
def extraer_articulos(xml_data: Optional[bytes]) -> List[Articulo]:
    ...
    for i_debug, ef_tag_debug in enumerate(estructuras_funcionales_tags[:3]):
    ...
    for i, ef_tag in enumerate(estructuras_funcionales_tags):
        ...
        if "artículo" not in tipo_parte and "articulo" not in tipo_parte:
            logger.debug(f"Omitiendo EstructuraFuncional XML #{i+1} (idParte: {id_parte_xml or 'N/A'}) porque tipoParte='{ef_tag.get('tipoParte', '')}' no es 'Artículo'.")
            continue

        logger.debug(f"Procesando EstructuraFuncional XML #{i+1} (idParte: {id_parte_xml or 'N/A'}, tipoParte='{ef_tag.get('tipoParte', '')}') como un Artículo.")

        texto_tag = ef_tag.find("Texto") 
        if not texto_tag or not texto_tag.text or not texto_tag.text.strip():
            logger.warning(f"EstructuraFuncional tipo Artículo XML #{i+1} (idParte: {id_parte_xml or 'N/A'}) no tiene etiqueta <Texto> o está vacía. Se omite.")
            continue
        
        texto_completo_articulo = texto_tag.text.strip()
        logger.debug(f"Texto completo extraído de <Texto> para EF #{i+1} (idParte: {id_parte_xml or 'N/A'}): '{texto_completo_articulo[:200]}...'")
        ...
        if match_numero_en_texto:
            ...
            display_temp = re.sub(r"^(artículo|articulo|art\.?)\s*", "", display_temp, flags=re.IGNORECASE).strip() # Quitar "Artículo "
            numero_display_extraido = display_temp.rstrip('.-').strip() # Quitar ".-" o "." o "-" al final
            ...
            if match_numero_en_texto.group(3) and match_numero_en_texto.group(3).strip():
                 ...
                 texto_neto_articulo = "" 
            logger.debug(f"EF #{i+1}: NumeroDisplay extraído y limpiado: '{numero_display_extraido}', Texto neto (inicio): '{texto_neto_articulo[:100]}...'")
        else:
            logger.warning(f"EF #{i+1}: No se pudo extraer un número de artículo del inicio del texto: '{texto_completo_articulo[:100]}...'. Se usará el texto completo como display para normalización.")
            numero_display_extraido = texto_completo_articulo.split('.')[0].split('-')[0].strip() # Intento básico
        ...
        if numero_id_interno == "s/n_error_normalizacion" or not numero_id_interno :
            logger.warning(f"EF Artículo XML #{i+1} (Display extraído='{numero_display_extraido}', idParte: {id_parte_xml or 'N/A'}): Normalización fallida. Se omite.")
            continue
        if numero_id_interno == "s/n" and numero_display_extraido != "S/N":
             logger.warning(f"EF Artículo XML #{i+1} (Display extraído='{numero_display_extraido}', idParte: {id_parte_xml or 'N/A'}): Normalización resultó en 's/n'. Revisar.")
        ...
        articulos_procesados.append(Articulo(
            articulo_display=numero_display_extraido.strip(), 
            articulo_id_interno=numero_id_interno,
            texto=texto_final_limpio, 
            referencias_legales=referencias_finales,
            id_parte_xml=id_parte_xml 
        ))
    
    logger.info(f"Extracción finalizada. {len(articulos_procesados)} artículos procesados y añadidos.")
    if not articulos_procesados and xml_data and len(estructuras_funcionales_tags) > 0 :
